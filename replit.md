...
- **Tree Visualization Filters with Color Muting** (2025-10-23): Interactive filters integrated into visualization controls panel for visual highlighting of accounts in D3 trees. Three filter criteria: **XLM Balance Range** (min/max), **Asset Balance Range** (sum of all asset values, min/max), and **Issuers Only** (accounts with ≥1 asset). Filters apply ONLY to D3 visualizations (both radial and tidy tree modes), NOT to table data. **Critical Design**: Filters use **color muting** instead of hiding nodes - accounts not matching criteria are dimmed (#1a1a2e fill, #2a2a3e stroke, 0.2 opacity for circles, 0.15 for text labels) but remain in the tree structure to preserve parent-child relationships. `window.shouldMuteNode()` evaluates filter criteria for each node during D3 rendering. Filter state persists via localStorage across page refreshes. Reset button clears all filters and localStorage. Cyberpunk-styled filter inputs with consistent glow effects.
