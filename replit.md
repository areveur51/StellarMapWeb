...
- **Sibling Visualization with Separated Tabs** (2025-10-23): Enhanced lineage tree displays siblings (other children of each creator) at every level alongside the direct lineage path. Intelligent color coding system: **Red links** (#ff3366) highlight the direct lineage path with 2.5px thickness and 90% opacity; **Gray links** (#888888) show siblings with 1.5px thickness and 50% opacity; **Cyan glow** (#00ffff) marks the searched account with enlarged radius and drop-shadow effect. **Separate Tabs**: "Account Lineage" tab shows ONLY the direct lineage path (10 accounts); "Siblings" tab displays all sibling accounts (175 siblings) separately with count header, **sorted by lineage order** (siblings of earlier creators appear first, with secondary sort by account creation date for deterministic ordering). Backend API endpoint `/api/lineage-with-siblings/` uses **chunked batch queries** (groups of 25 to respect Cassandra's IN limit) reducing total queries from ~200 to ~15 for 185 accounts (configurable max_siblings_per_level, default 50). Frontend `transformSiblingsToLineage()` separates data into `account_lineage_data` (direct path only) and `siblings_data` (siblings only) with metadata flags (is_lineage_path, is_sibling, is_searched_account) for D3.js rendering. Works in both radial and horizontal tidy tree modes.
...
- **Tree Visualization Filters with Color Muting** (2025-10-24): Interactive filters integrated into visualization controls panel for visual highlighting in D3 trees. **Filter Criteria**: (1) **Issuer Balance** (min/max range for ISSUER nodes only, based on XLM balance), (2) **Asset Balance** (min/max range for ASSET nodes only, based on individual asset balance), (3) **Issuers Only** checkbox (accounts with ≥1 asset). **Slider Increment Selector**: Dropdown allowing users to choose slider step size (5, 10, 50, 100, 1000, 5000, 10000) for precise control. Filters apply ONLY to D3 visualizations (both radial and tidy tree modes), NOT to table data. **Critical Design**: Filters use **color muting** instead of hiding nodes - nodes not matching criteria are dimmed (#1a1a2e fill, #2a2a3e stroke, 0.2 opacity for circles, 0.15 for text labels) but remain in the tree structure to preserve parent-child relationships. **Scoped Filtering**: Issuer Balance filters ONLY issuer/account nodes; Asset Balance filters ONLY asset nodes (yellow). `window.shouldMuteNode()` evaluates filter criteria for each node during D3 rendering. All filter settings and increment selection persist via localStorage across page refreshes. Reset button clears all filters and localStorage. Cyberpunk-styled dual range sliders with dynamic value display (0, 1k, 100k, ∞) and consistent glow effects.
