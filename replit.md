...
- **BigQuery-Based Data Collection (Primary)**: Main pipeline uses Stellar's public BigQuery/Hubble dataset for efficient data retrieval. Processes accounts in 50-90 seconds (2-3x faster than API-based approach). Retrieves account data, assets, creator, and up to 100,000 child accounts using 4 optimized queries. No API rate limits. ~$0.10-0.50 per 1,000 accounts (within free tier for most use cases).
