...
- **Sibling Visualization with Color Coding** (2025-10-23): Enhanced lineage tree displays siblings (other children of each creator) at every level alongside the direct lineage path. Intelligent color coding system: **Red links** (#ff3366) highlight the direct lineage path with 2.5px thickness and 90% opacity; **Gray links** (#888888) show siblings with 1.5px thickness and 50% opacity; **Cyan glow** (#00ffff) marks the searched account with enlarged radius and drop-shadow effect. Backend API endpoint `/api/lineage-with-siblings/` fetches lineage path + siblings in a single query (configurable max_siblings_per_level, default 50). Frontend transforms sibling data with metadata flags (is_lineage_path, is_sibling, is_searched_account) for D3.js rendering. Works in both radial and horizontal tidy tree modes.
