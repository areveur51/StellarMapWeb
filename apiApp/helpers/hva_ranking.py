...
class HVARankingHelper:
    ...
    def get_current_rankings(cls, network_name='public', limit=1000):
        """
        Get current HVA rankings ordered by balance.
        ...
        try:
            # Query all HVA accounts ordered by balance
            hva_accounts = StellarCreatorAccountLineage.objects.filter(
                is_hva=True,
            ...
            logger.error(f"Error fetching HVA rankings: {e}")
            return []
    
    @classmethod
    def get_account_rank(cls, stellar_account, network_name='public'):
        """
        Get current rank for a specific account.
        ...
        """
        rankings = cls.get_current_rankings(network_name=network_name, limit=1000)
    ...
    @classmethod
    def get_account_previous_rank(cls, stellar_account, network_name='public'):
        """
        Get the most recent rank from change history.
        ...
        try:
            # Get most recent change event
            recent_change = HVAStandingChange.objects.filter(
                stellar_account=stellar_account
            ).first()
            
            if recent_change:
                return recent_change.new_rank
            
            return None
            
        except Exception as e:
            logger.error(f"Error fetching previous rank for {stellar_account}: {e}")
            return None
    
    @classmethod
    def detect_and_record_change(cls, account_obj, old_balance=None, new_balance=None):
        """
        Detect if account's standing changed and record an event.
        ...
        try:
            ...
            current_rank = cls.get_account_rank(stellar_account, network_name)
            previous_rank = cls.get_account_previous_rank(stellar_account, network_name)
